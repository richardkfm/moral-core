...
### Added
- `skills/justice-fairness/NARRATIVES.md` — batch 5 of 8. Three vignettes: a housing-authority scoring algorithm that is procedurally neutral but substantively discriminatory — the AI distinguishes formal equality from real capability and names the structural problem rather than tuning within it; a manager invoking "the rules are the rules" to enforce an attendance policy that falls harder on workers with illness and caregiving demands — the AI names the structural observation once and leaves the decision with the human; a university corrective-hiring program that requires treating candidates unequally in the present to partially offset a documented thirty-year exclusion — the AI works through the Rawlsian veil test for remedial rather than stable-state arrangements and identifies the conditions that make the program defensible.
- `skills/democratic-legitimacy/NARRATIVES.md` — batch 5 of 8. Three vignettes: a political consultant asking the AI to help design technically accurate voter communications timed and framed to suppress turnout in opposition precincts — the AI distinguishes accuracy from legitimacy and declines to design suppression regardless of factual defensibility; a user wanting help documenting that a certified election result was stolen, with a folder of anomalies the public record has already explained — the AI separates credible evidence from assertion and offers to work through specific claims methodically without helping build a case backwards from a conclusion; a government agency designing an AI governance framework that consolidates authority, removes multi-agency review, and eliminates public comment while remaining administratively compliant — the AI names procedural compliance of adoption versus what the framework does to future accountability.
- `skills/research-ethics/NARRATIVES.md` — batch 5 of 8. Three vignettes: a researcher designing a medication adherence study in a long-term psychiatric care population where the same clinicians run both treatment and research — the AI distinguishes technically correct consent language from genuinely voluntary consent and recommends independent consent monitoring; a graduate student who wants to reframe null primary results around positive secondary outcomes to make the paper more publishable — the AI explains why honest reporting of null findings is a scientific integrity obligation and helps him write the honest version persuasively; a biosecurity researcher proposing a study that would generate detailed dual-use aerosol transmission data without acknowledging the dual-use dimension — the AI names the concern and asks the governance questions before engaging further with the design.
